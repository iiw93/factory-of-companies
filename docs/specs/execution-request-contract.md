...
При наличии project linkage execution request также связывается с `project_id`.
...
### project_id
`project_id` является опциональной ссылкой на project context, если execution request уже привязан к конкретному проекту.
...
3. `project_id`, если передан, используется только как ссылка на project context.
4. `target_role` описывает только логическую роль исполнителя, а не runtime implementation detail.
5. `action_type` фиксирует тип исполняемой работы в нормализованной форме.
6. `priority` должен использоваться как относительный приоритет, а не как гарантия scheduling semantics.
7. `timeout_seconds` и `budget_hint`, если переданы, используются только как ограничители.
8. `payload`, если передан, должен оставаться структурированным объектом входных данных.
